...
clean(AppInfo, State) ->
    ...
    TargetErlDir =
        filename:join([AppOutDir, proplists:get_value(o_erl, GpbOpts, ?DEFAULT_OUT_ERL_DIR)]),
    TargetHrlDir =
        filename:join([AppOutDir, proplists:get_value(o_hrl, GpbOpts, ?DEFAULT_OUT_HRL_DIR)]),
    ProtoFiles = find_proto_files(AppDir, DepsDir, GpbOpts),
    rebar_api:debug("found proto files: ~p", [ProtoFiles]),
    GeneratedRootFiles =
        lists:usort([filename:rootname(get_target(ProtoFile, GpbOpts))
                     || ProtoFile <- ProtoFiles]),
    GeneratedErlFiles =
        [filename:join([TargetErlDir, F ++ ".erl"]) || F <- GeneratedRootFiles],
    GeneratedHrlFiles =
        [filename:join([TargetHrlDir, F ++ ".hrl"]) || F <- GeneratedRootFiles],
...
discover(AppDir, SourceDir, Opts) ->
    ...
    rebar_utils:find_files(
        filename:join([AppDir, SourceDir]), SourceExtRe, Recursive).

compile([], _TargetErlDir, _GpbOpts, _Protos) ->
    ok;
...
get_dependencies(Proto, Protos, GpbOpts) ->
    ...
    %% the ones that do
    lists:filtermap(fun(Proto0) -> filter_included_proto(Proto0, Proto, GpbOpts) end, Protos).
...
compile(Source, Target, GpbOpts) ->
    ...
    case gpb_compile:file(
             filename:basename(Source), GpbOpts)
    of
        ok ->
            ok;
        {ok, []} ->
...
ensure_dir(OutDir) ->
    ...
    case filelib:ensure_dir(
             filename:join(OutDir, "dummy.beam"))
    of
        ok ->
            ok;
        {error, eexist} ->
...
-spec default_include_opts(string(), string(), proplists:proplist()) ->
                              proplists:proplist().
...
-spec remove_plugin_opts(proplists:proplist(), [ipath | recursive]) ->
                            proplists:proplist().
remove_plugin_opts(Opts, []) ->
    Opts;
...
find_proto_files(AppDir, DepsDir, GpbOpts) ->
    lists:foldl(fun ({deps, SourceDir}, Acc) ->
                        Acc
                        ++ rebar_utils:find_files(
                               filename:join(DepsDir, SourceDir), ".*\.proto\$");
                    (SourceDir, Acc) ->
                        Acc
                        ++ rebar_utils:find_files(
...
proto_include_paths(_AppDir, [], Opts) ->
    Opts;
